...
class Menu extends Model
{
    ...
    public function getPadres($front)
    {
        if ($front) {
            return $this->whereHas('roles', function ($query) {
                $query->where('id_rol', session()->get('id_rol'))->orderby('menu_id');
            })->where('estado', 1)
                ->orderby('menu_id')
                ->orderby('orden')
                ->get()
    ...
    public function guardarOrden($menu)
    {
        ...
        foreach ($menus as $var => $value) {
            $this->where('id_menu', $value->id)->update(['menu_id' => 0, 'orden' => $var + 1]);
            if (!empty($value->children)) {
                foreach ($value->children as $key => $vchild) {
                    $update_id = $vchild->id;
                    $parent_id = $value->id;
                    $this->where('id_menu', $update_id)->update(['menu_id' => $parent_id, 'orden' => $key + 1]);
